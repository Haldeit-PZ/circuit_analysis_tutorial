...
def main():
    ...
    parser.add_argument('--plot_bode_data', action='store_true', help='Plot PyZero responses')
    parser.add_argument('--plot_noise', action='store_true', help='Plot PyZero noise responses')
    parser.add_argument('--plot_noise_data', action='store_true', help='Plot PyZero noise responses')
    ...
    process_files(input_file_content, data_file_content, args.plot_pyzero, args.plot_noise, args.plot_bode_data, args.plot_noise_data)

def process_files(input_file_content, data_file_content, plot_pyzero, plot_noise, plot_bode_data, plot_noise_data):
    ...
    if plot_pyzero:
        ...
        solution_bode = analysis.calculate(frequencies=frequencies, input_type="voltage", node=nin)

        res = solution_bode.get_response(nin, nout)
        magres = res.db_magnitude
        phares = res.phase

        magnitude_z = np.array(magres)
        phase_z = np.array(phares)
        plot = solution_bode.plot_responses(sink=nout)
        plot.show()

    if plot_noise:
        analysis = AcNoiseAnalysis(circuit=circuit)
        solution_noise = analysis.calculate(frequencies=frequencies, input_type="voltage", node=nin,
                              sink=nout, incoherent_sum=True)

        plot = solution_noise.plot_noise(sink=nout)
        response = solution_noise.get_noise_sum(sink = nout)
        plot.show()
        print(response.spectral_density)
    ## Experimental Code
        ...
        if data_file_content:
            ...
            for line in lines:
                if start_parsing:
                    ...
                    # Parse the data
                    parts = line.split()
                    parts = [item[:-1] if item.endswith(',') else item for item in parts]
    ...
    if plot_noise_data:
        if data_file_content:
            ...
            res = solution_noise.get_noise_sum(sink = nout)
            nosres = res.spectral_density

            noise_z = np.array(nosres)
            ...
            # Plots
            fig, ((s1, r1)) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)

            # Plot 1: Magnitude vs Frequency
            s1.semilogx(frequency_e, noise_e, 'b', label='Original Data', color='#f0a963ff')
            s1.semilogx(frequency_e, noise_z, 'b', label="PyZero Data")
            s1.set_ylabel('Noise(V/rt(Hz))')
            s1.set_title('Noise vs Frequency')
            #s1.set_yticks([-40, -20, 0])
            s1.grid()
            s1.grid(which='minor', ls='--', alpha=0.5)
            s1.legend()
            ...
            # Plot 3: Magnitude Residuals
            r1.semilogx(frequency_e, np.array(noise_e) - noise_z, '#f0a963ff')
            r1.set_ylabel('Noise(V/rt(Hz))')
            r1.set_title('Noise Residuals')
